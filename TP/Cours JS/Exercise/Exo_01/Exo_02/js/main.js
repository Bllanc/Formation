...
function testDate() {
    let datedivisee = new Date().toISOString().split("T");
    let dateElements = datedivisee[0].split("-");
    let heureElements = datedivisee[1].split(".")[0].split(":");
  
    document.write(`
    aujourd'hui, on est le ${dateElements[2]}/${dateElements[1]}/${dateElements[0]}, 
    il est ${heureElements[0]}h${heureElements[1]}mn${heureElements[2]}s
    `)
  }
  testDate();